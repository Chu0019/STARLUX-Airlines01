...
const WATCH_CONFIG_URL = "/api/watch/config";
const WATCH_STORAGE_KEY = "starlux-watch-flights-v1";
const WATCH_CONFIG_SYNC_INTERVAL = 3 * 1000;
...
let watchConfigSignature = "";

if (flightInput) {
  flightInput.value = trackedFlightCodes.join(",");
}

watchConfigSignature = getTrackedFlightSignature(trackedFlightCodes);
...
function getTrackedFlightSignature(flightCodes) {
  return flightCodes.join(",");
}

function setTrackedFlightCodes(nextFlights) {
  trackedFlightCodes = parseFlightCodes(Array.isArray(nextFlights) ? nextFlights.join(",") : nextFlights);
  watchConfigSignature = getTrackedFlightSignature(trackedFlightCodes);
...
async function fetchWatchConfig() {
  const response = await fetch(WATCH_CONFIG_URL, { cache: "no-store" });

  if (!response.ok) {
    throw new Error("Unable to load watch config");
  }

  return response.json();
}

async function saveWatchConfig(nextFlights) {
  const response = await fetch(WATCH_CONFIG_URL, {
    method: "POST",
    headers: {
      "content-type": "application/json",
    },
    body: JSON.stringify({ flights: nextFlights }),
  });

  if (!response.ok) {
    throw new Error("Unable to save watch config");
  }

  return response.json();
}

async function syncWatchConfig(reloadOnChange = false) {
  if (isConfigSyncing) return false;
  isConfigSyncing = true;

  try {
    const config = await fetchWatchConfig();
    const nextFlights = parseFlightCodes(Array.isArray(config.flights) ? config.flights.join(",") : config.flights);
    const nextSignature = getTrackedFlightSignature(nextFlights);
    const changed = nextSignature !== watchConfigSignature;

    if (changed) {
      setTrackedFlightCodes(nextFlights);
      if (reloadOnChange) {
        await loadTdxFlights(false);
      }
    }

    const configStatusElement = document.querySelector("#configStatus");
    if (configStatusElement) {
      configStatusElement.textContent = nextFlights.length ? `${nextFlights.length} 班` : "未設定";
    }
...
async function forceRefreshFlights() {
  if (!forceRefreshButton || !forceRefreshStatusElement) return;
...
function render() {
  ...
  if (updatedAtElement) updatedAtElement.textContent = `${tdxRefreshMinutes} 分鐘`;
  if (fr24UpdatedAtElement) fr24UpdatedAtElement.textContent = `${fr24RefreshMinutes} 分鐘`;
  if (tdxLastUpdatedAtElement) tdxLastUpdatedAtElement.textContent = `上次更新 ${lastTdxRefreshAt ? formatClock(lastTdxRefreshAt) : "--:--"}`;
  if (fr24LastUpdatedAtElement) fr24LastUpdatedAtElement.textContent = `上次更新 ${lastFr24RefreshAt ? formatClock(lastFr24RefreshAt) : "--:--"}`;

  if (forceRefreshButton && forceRefreshStatusElement && forceCooldownMinutes > 0) {
    forceRefreshButton.disabled = true;
    forceRefreshButton.textContent = `${forceCooldownMinutes} 分鐘`;
    forceRefreshStatusElement.textContent = "強制更新冷卻中";
  } else if (forceRefreshButton && forceRefreshStatusElement && !forceRefreshButton.disabled) {
    forceRefreshButton.textContent = "強制更新";
...
if (watchForm && flightInput) {
  watchForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const nextFlights = parseFlightCodes(flightInput.value);
    flightInput.value = nextFlights.join(",");

    if (forceRefreshStatusElement) {
      forceRefreshStatusElement.textContent = "正在保存追蹤航班";
    }

    void (async () => {
      try {
        const config = await saveWatchConfig(nextFlights);
        setTrackedFlightCodes(config.flights || nextFlights);
        flights = [];
        fr24Flights = [];
        if (forceRefreshStatusElement) {
          forceRefreshStatusElement.textContent = "追蹤名單已保存";
        }
        await loadTdxFlights(false);
      } catch {
        if (forceRefreshStatusElement) {
          forceRefreshStatusElement.textContent = "保存失敗";
        }
      }
    })();
  });
...
async function syncWatchConfigIfChanged() {
  const changed = await syncWatchConfig(false);
...
setInterval(syncWatchConfigIfChanged, WATCH_CONFIG_SYNC_INTERVAL);
